...
void off( control_unit_t* unit, uint8_t pinToOff, uint8_t safeTime )
{
	...
	{
		send_state(ERROR, "already off"); // Cannot be turned on, because already off
		return;
	}
	
	if( unit->isBad )
	{
		send_state(ERROR, "pin corrupted"); // Pin courrupted and disabled
		return;	
	}
...
void on( control_unit_t* unit, uint8_t pinToOn )
{
	...
	{
		send_state(ERROR, "already on"); // Cannot be turned on, because already on
	}
	else if( unit->isBad )
	{
		send_state(ERROR, "pin corrupted"); // Pin courrupted and disabled
	...
	else 
	{
		...
		{
			asm("sei");
			send_state(ERROR, "too often"); // Cannot be turned on, because too often request
			return;
		}
